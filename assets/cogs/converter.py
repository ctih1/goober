...
Fahrenheit: Unit = {"name": "Fahrenheit", "shorthand": " °F"}

Celsius: Unit = {"name": "Celsius", "shorthand": " °C"}

Feet: Unit = {"name": "Feet", "shorthand": " ft"}

Meters: Unit = {"name": "Meter", "shorthand": " m"}

Inch: Unit = {"name": "Inch", "shorthand": '"'}

Centimeter: Unit = {"name": "Centimeter", "shorthand": " cm"}

Mile: Unit = {"name": "Mile", "shorthand": " mile(s)"}

Kilometer: Unit = {"name": "Kilometer", "shorthand": "km"}

Liters: Unit = {"name": "Liter", "shorthand": " L"}

Gallons: Unit = {"name": "Gallon", "shorthand": " gal"}

Kilogram: Unit = {"name": "Kilogram", "shorthand": " kg"}

Pounds: Unit = {"name": "Pound", "shorthand": " lbs"}

Grams: Unit = {"name": "Gram", "shorthand": " g"}
...
Reactions = [
    ...
    "in stupid units",
    "since you don't know how to google...",
    "for our friend across the pond",
    "in units that suck less",
    "",
]
...
class Converters:
    ...
    def from_fahrenheit(value: float) -> ConvertedValue:
        return {
            "metric": {"unit": Celsius, "value": (value - 32) / 1.8},
            "imperial": None,
        }

    @staticmethod
    def from_meters(value: float) -> ConvertedValue:
        return {"metric": None, "imperial": {"unit": Feet, "value": value * 3.28084}}
    ...
    def from_inches(value: float) -> ConvertedValue:
        return {
            "metric": {"unit": Centimeter, "value": value * 2.54},
            "imperial": {"unit": Feet, "value": value * 0.0833333333},
        }
    ...
    def from_kilometers(value: float) -> ConvertedValue:
        return {
            "metric": None,
            "imperial": {"unit": Mile, "value": value * 0.621371192},
        }

    @staticmethod
    def from_miles(value: float) -> ConvertedValue:
        return {
            "metric": {"unit": Kilometer, "value": value / 0.621371192},
            "imperial": None,
        }
    ...
    def from_ms(value: float) -> ConvertedValue:
        return {
            "metric": {"value": value * 3.6, "unit": to_speed_unit(Kilometer)},
            "imperial": {"unit": to_speed_unit(Mile), "value": value * 2.23693629},
        }

    @staticmethod
    def from_gallons(value: float) -> ConvertedValue:
        return {
            "metric": {"value": value * 3.78541178, "unit": Liters},
            "imperial": None,
        }
    ...
    def from_liters(value: float) -> ConvertedValue:
        return {
            "metric": None,
            "imperial": {"value": value * 0.264172052, "unit": Gallons},
        }
    ...
    def from_kilograms(value: float) -> ConvertedValue:
        return {
            "metric": None,
            "imperial": {"value": value * 2.20462262, "unit": Pounds},
        }

    @staticmethod
    def from_pounds(value: float) -> ConvertedValue:
        return {
            "metric": {"unit": Kilogram, "value": value * 0.45359237},
            "imperial": None,
        }
    ...
    def from_grams(value: float) -> ConvertedValue:
        return {
            "metric": None,
            "imperial": {"unit": Ounces, "value": value * 0.0352739619},
        }

    @staticmethod
    def from_ounces(value: float) -> ConvertedValue:
        return {
            "metric": {"unit": Grams, "value": value * 28.349523125},
            "imperial": None,
        }
...
class Converter(commands.Cog):
    def __init__(self, bot):
        ...
        self.regexes: Dict[re.Pattern, Callable] = {
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(c|\*c)(\s|$)", re.IGNORECASE
            ): Converters.from_celsius,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(f|\*f)(\s|$)", re.IGNORECASE
            ): Converters.from_fahrenheit,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(m|meters)(\s|$)", re.IGNORECASE
            ): Converters.from_meters,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(cm|centimeter|centimeters)(\s|$)",
                re.IGNORECASE,
            ...
            ): Converters.from_miles,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(km|kilometers|kilometer)(\s|$)",
                re.IGNORECASE,
            ): Converters.from_kilometers,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(km\/h|kmh|kph)(\s|$)", re.IGNORECASE
            ): Converters.from_kmh,
            re.compile(
            ...
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(gal|gallons|gallon)(\s|$)", re.IGNORECASE
            ): Converters.from_gallons,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(l|liter|liters)(\s|$)", re.IGNORECASE
            ): Converters.from_liters,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(kg|kilos|kilo|kilograms|kilogram)(\s|$)",
                re.IGNORECASE,
            ): Converters.from_kilograms,
            re.compile(
                r"(?:\s|^)(-?[0-9(.?|,?)]+)\s?(pounds|lbs|lb)(\s|$)", re.IGNORECASE
            ): Converters.from_pounds,
            re.compile(
    ...
    def __format_response(self, converted_values: List[ConvertedValue]) -> str:
        ...
        for i, converted in enumerate(converted_values):
            temp_line: str = (
                ", and "
                if (i == len(converted_values) - 1 and i != 0)
                else ", " if i != 0 else ""
            )
            data_line: str = ""
            ...
            for unit, data in converted.items():
                ...
                data_line += (
                    f"{' or ' if data_line else ''}**{round(value,2)}{shorthand}**"
                )
