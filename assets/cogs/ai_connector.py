    ...
    async def do(self, ctx: Context, *, content: str):
        ...
        async with ctx.typing():
            response: Response = await requests_async.post(
                "http://192.168.32.88:3800/generate",
                json={"prompt": content, "person": person},
                headers={"X-Auth": os.environ.get("AI_KEY")},
            )
        if response.status_code != 200:
            await send_message(
                ctx, f"Failed to generateo: {response.status_code} {response.text}"
            )
            return
        ...
        if comparing:
            ...
            await send_message(
                ctx, response.json().get("generated", "Failed to generate???")
            )
