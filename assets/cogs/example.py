...
class Example(commands.Cog):
    ...
    async def random_number(
        self, ctx: commands.Context, minimum: int | None, maximum: int | None
    ):  # every argument after ctx is a part of the command, aka "g.random_number 0 5" would set minimum as 0 and maximum as 5
    ...
    @commands.command()
    async def ban_user(
        self, ctx: commands.Context, target: discord.Member | None, reason: str | None
    ):
        if target is None:
    ...
    @commands.command()
    async def change_hello_message(
        ...
        default_settings: SettingsType = (
            {  # use default_settings = { if you didnt define the types
                "intro": {"message": "Hello user!"},
