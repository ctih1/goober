...
class SpotifyLarper(commands.Cog):
    def __init__(self, bot: discord.ext.commands.Bot):
        ...
        self.discord_activity: discord.Activity = discord.Activity(
            type=discord.ActivityType.listening, name="Not ready 2 larp"
        )
    ...
    async def now_larping(self, ctx: commands.Context):
        ...
        lyrics: List[str] = [
            self.current_lyrics[min_index],
            styling + self.current_lyrics[self.current_lyric_index] + styling,
            self.current_lyrics[max_index],
        ]

        embed = discord.Embed(
            title="Currently Larping",
            description=f'## {self.current_activity.title}\nby **{self.current_activity.artist}** on album "{self.current_activity.album}"\n‌',
        )
    ...
    async def on_presence_update(
        ...
        for activity in after.activities:
            if not isinstance(activity, discord.activity.Spotify):
                continue
        ...
        matches: List[LRCLIBResponse] = await LRCAPI.search_song(
            f"{target_activity.title} {target_activity.artist}"
        )
        ...
        self.discord_activity = discord.Activity(
            ...
            timestamps={"start": round(time.time() * 1000)},
        )
