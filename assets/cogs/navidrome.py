...
class NaviDrome(commands.Cog):
    ...
    async def song(self, ctx: commands.Context, *args):
        ...
        await message.edit(
            content=f"Found matches. Reply with the number:\n\n{response_string}"
        )
    ...
    async def download_music(self, match: VideoEntry, message: discord.Message):
        if match["lengthSeconds"] > 7 * 60:
            await message.edit(
                content="Ayo sussy boy that video is looking long ash im not letting you download that shit"
            )
            return

        settings = settings_manager.get_plugin_settings("youtube", default_settings)
        video = AsyncYouTube(
            url=f"https://youtube.com/watch?v={match['videoId']}",
            client=(
                "WEB"
        ...
        await loop.run_in_executor(
            ...
            lambda: stream.download(
                f"data/youtube/{match['videoId']}", filename="audio.webm"
            ),
        )
        ...
        res = await requests_async.get(
            match["videoThumbnails"][3]["url"], allow_redirects=True
        )
        ...
        try:
            res = await requests_async.post(
                ...
                headers={
                    "Authorization": f"Basic {base64.b64encode(os.environ['NAVIDROME_AUTH'].encode()).decode()}"
                },
        ...
        if res.status_code != 200:
            await message.edit(
                content=f"Upload failed with status code {res.status_code}"
            )
            return
