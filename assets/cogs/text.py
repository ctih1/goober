...
class Text(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.spotify_header_font = ImageFont.truetype(
            "assets/fonts/resotyc-Regular.ttf", 54
        )
        self.spotify_artist_font = ImageFont.truetype(
            "assets/fonts/gevher-black.otf", 58
        )
    ...
    async def this_is(self, ctx: commands.Context, *args):
        ...
        image.paste(
            tuple([round(r * 255) for r in hsv_to_rgb(random.random(), 0.7, 0.9)]),
            (0, round(image.size[1] / 2), image.size[0], image.size[1]),
        )
        ...
        draw.text(
            (image.size[0] / 2 - top_length, 32),
            top_text,
            font=self.spotify_header_font,
            fill="black",
        )
        draw.text(
            (image.size[0] / 2 - self.spotify_artist_font.getlength(artist) / 2, 430),
            artist,
            font=self.spotify_artist_font,
            fill="black",
        )
        ...
        spotify_logo = Image.open(os.path.join(self.assets, "spotify.png")).convert(
            "RGBA"
        )
        ...
        x, y = (
            image.size[0] // 2 - artist_image.size[0] // 2,
            image.size[0] // 2 - artist_image.size[1] // 2,
        )
        image.paste(
            artist_image, (x, y, x + artist_image.width, y + artist_image.height)
        )

        image.save(os.path.join(self.cache, "this_is.png"))

        await send_message(
            ctx, file=discord.File(os.path.join(self.cache, "this_is.png"))
        )
