...
default_settings: SettingsType = {"latitude": 30, "longtitude": 0}
...
CO2_TRESHOLDS: Dict[int, TresholdValue] = {
    400: {"label": "Great", "emoji": "🔵"},
    600: {"label": "Good", "emoji": "🟢"},
    800: {"label": "OK", "emoji": "🟢"},
    1000: {"label": "Suboptimal", "emoji": "🟡"},
    1300: {"label": "Bad", "emoji": "🔴"},
    1800: {"label": "Very bad", "emoji": "🟣"},
}

TEMP_TRESHOLDS: Dict[int, TresholdValue] = {
    16: {"label": "Cold", "emoji": "🔵"},
    18: {"label": "Optimal", "emoji": "🟢"},
    21: {"label": "Warm", "emoji": "🟡"},
    24: {"label": "Hot", "emoji": "🔴"},
}

HUMIDITY_TRESHOLDS: Dict[int, TresholdValue] = {
    0: {"label": "Too dry", "emoji": "🟡"},
    35: {"label": "Optimal", "emoji": "🟢"},
    65: {"label": "Too damp", "emoji": "🟡"},
}

RESISTANCE_TRESHOLDS: Dict[int, TresholdValue] = {
    20_000: {"label": "Bad", "emoji": "🟡"},
    90_000: {"label": "OK", "emoji": "🟡"},
    120_000: {"label": "Good", "emoji": "🟢"},
}

PM25_TRESHOLDS: Dict[int, TresholdValue] = {
    0: {"label": "Excellent", "emoji": "🔵"},
    6: {"label": "Good", "emoji": "🟢"},
    12: {"label": "OK", "emoji": "🟡"},
    35: {"label": "Unhealthy", "emoji": "🔴"},
    60: {"label": "Very unhealthy", "emoji": "🟣"},
}
...
PM100_TRESHOLDS: Dict[int, TresholdValue] = {
    ...
    12: {"label": "Good", "emoji": "🟢"},
    24: {"label": "OK", "emoji": "🟡"},
    75: {"label": "Unhealthy", "emoji": "🔴"},
    120: {"label": "Very unhealthy", "emoji": "🟣"},
}

OUTDOOR_TEMP_TRESHOLDS: Dict[int, TresholdValue] = {
    -50: {"label": "Extremely frigid", "emoji": "⚫"},
    -25: {"label": "Very frigid", "emoji": "🟣"},
    -20: {"label": "Very cold", "emoji": "🔵"},
    -15: {"label": "Cold", "emoji": "🔵"},
    -10: {"label": "Mildly cold", "emoji": "🔵"},
    -5: {"label": "Chilly", "emoji": "⚪"},
    0: {"label": "Cool", "emoji": "⚪"},
    5: {"label": "Brisk", "emoji": "🟢"},
    10: {"label": "Mild", "emoji": "🟢"},
    15: {"label": "Warm", "emoji": "🟡"},
    20: {"label": "Very warm", "emoji": "🟡"},
    25: {"label": "Hot", "emoji": "🟠"},
    30: {"label": "Very hot", "emoji": "🔴"},
}

OUTDOOR_HUMIDITY_TRESHOLDS: Dict[int, TresholdValue] = {
    0: {"label": "Extremely dry", "emoji": "🟠"},
    15: {"label": "Very dry", "emoji": "🟠"},
    30: {"label": "Dry", "emoji": "🟡"},
    40: {"label": "Comfortable", "emoji": "🟢"},
    69: {"label": "Slightly humid", "emoji": "🟢"},
    75: {"label": "Humid", "emoji": "🔵"},
    85: {"label": "Very humid", "emoji": "🔵"},
    95: {"label": "Extremely humid", "emoji": "🟣"},
}

SUN_POSITION_TRESHOLD: Dict[int, TresholdValue] = {
    -100: {"label": "Night", "emoji": "🌌"},
    -18: {"label": "Astronomical twilight", "emoji": "🌙"},
    -12: {"label": "Nautical twilight", "emoji": "⭐"},
    -6: {"label": "Civil twilight", "emoji": "🌃"},
    -2: {"label": "Sunrise or -set", "emoji": "🌅"},
    2: {"label": "Day", "emoji": "☀️"},
}

PRESSURE_TRESHOLS: Dict[int, TresholdValue] = {
    900: {"label": "Extremely low pressure", "emoji": "🔴"},
    980: {"label": "Very low pressure", "emoji": "🟠"},
    995: {"label": "Low pressure", "emoji": "🟡"},
    1005: {"label": "Normal pressure", "emoji": "🟢"},
    1015: {"label": "High pressure", "emoji": "🔵"},
...
class Climate(commands.Cog):
    ...
    @staticmethod
    def get_ranking(
        current_value: float, tresholds: Dict[int, TresholdValue]
    ) -> TresholdValue:
        """
        Gets the ranking (e.g good, bad, dangerous) for a given value
        """
        found_treshold: TresholdValue = {"emoji": "", "label": ""}
    ...
    def format_embed(
        label: str,
        unit: str,
        value: Tuple[float, float] | Tuple[float, ...],
        treshold: Dict[int, TresholdValue],
    ) -> dict:
    ...
    async def get_prometheus_data(
        ...
        for data in now_json["data"]["result"]:
            offset_data = [
                obj
                for obj in offset_json["data"]["result"]
                if obj["metric"]["__name__"] == data["metric"]["__name__"]
            ]

            if len(offset_data) != 1:
                logger.warning(f"Weird match with {data}")
                values[data["metric"]["__name__"]] = (
                    float(data["value"][1]),
                    float(data["value"][1]),
                )

            values[data["metric"]["__name__"]] = (
                float(offset_data[0]["value"][1]),
                float(data["value"][1]),
            )
    ...
    def get_sun_angle(at: datetime.datetime) -> float:
        ...
        result = math.degrees(
            math.asin(
                math.sin(declination) * math.sin(math.radians(settings["latitude"]))
    ...
    async def generate_outdoor_embed() -> discord.Embed:
        data = await Climate.get_prometheus_data(
            ["mc2p5", "mc10p0", "temp", "humidity", "climate_pressure"]
        )
        ...
        pressure = tuple(
            [
                data["climate_pressure"][i] * math.pow((1 - 119 / 44330), -5.225)
                for i in range(2)
            ]
        )

        embed.add_field(
            **Climate.format_embed("PM2.5", "µg/m³", data["mc2p5"], PM25_TRESHOLDS)
        )
        embed.add_field(
        ...
        embed.add_field(
            **Climate.format_embed(
                "Relative Humidity", "%", data["humidity"], OUTDOOR_HUMIDITY_TRESHOLDS
            )
        )
        embed.add_field(
            **Climate.format_embed("Air Pressure", "hPa", pressure, PRESSURE_TRESHOLS)
        )
        embed.add_field(
            **Climate.format_embed(
                "Sun angle",
                "°",
                (
                    Climate.get_sun_angle(
                        datetime.datetime.now() - datetime.timedelta(minutes=30)
                    ),
                    Climate.get_sun_angle(datetime.datetime.now()),
                ),
                SUN_POSITION_TRESHOLD,
            )
        )
    ...
    async def generate_indoor_embed() -> discord.Embed:
        ...
        calculated_temp = tuple(
            [
                data["climate_scd40_temp"][i]
                - ((data["climate_temp"][i] - data["climate_temp_adjusted"][i]) / 2)
                for i in range(2)
            ]
        )
        air_humidity = tuple(
        ...
        embed.add_field(
            **Climate.format_embed(
                "CO2", "PPM", data["climate_carbon_dioxide"], CO2_TRESHOLDS
            )
        )
        embed.add_field(
        ...
        embed.add_field(
            **Climate.format_embed(
                "Air Resistance",
                "Ω",
                data["climate_air_resistance"],
                RESISTANCE_TRESHOLDS,
            )
        )
        embed.set_footer(text="▲: value increasing, ▼: value decreasing")
